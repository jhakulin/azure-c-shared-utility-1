...
static void on_underlying_io_open_complete(void* context, IO_OPEN_RESULT io_open_result, char* errorReason)
{
    TLS_IO_INSTANCE* tls_io_instance = (TLS_IO_INSTANCE*)context;
    ...
    else
    {
        if (io_open_result != IO_OPEN_OK)
        {
            ...
            {
                tls_io_instance->on_io_open_complete(tls_io_instance->on_io_open_complete_context, IO_OPEN_ERROR, errorReason);
            }
        }
        else
        {
                ...
                case SEC_E_OK:
                    ...
                    else
                    {
                        if (tls_io_instance->tlsio_state == TLSIO_STATE_HANDSHAKE_CLIENT_HELLO_SENT)
                        {
                            ...
                            if (tls_io_instance->on_io_open_complete != NULL)
                            {
                                tls_io_instance->on_io_open_complete(tls_io_instance->on_io_open_complete_context, IO_OPEN_OK, NULL);
                            }
                        }
                        else
                        {
